...
function createCardTextureMaterial(color, value) {
    ...
    var pngFallbackPath = 'assets/textures/cards/card_' + color + '_' + value + '.png';
    var image = new Image();
            ...
            Object.keys(piles).forEach(function(id) {
                if (id.startsWith('dutch_pile_')) {
                    ...
                    if (id === nearestPileId) {
                        ...
                        var pile = null;
                        if (gameState && gameState.piles && typeof gameState.piles.get === 'function') {
                        ...
                        if (pile && pile.cardStack && pile.cardStack.length > 0) {
                            ...
                            if (gameState && gameState.cards && typeof gameState.cards.get === 'function') {
...
window.syncColyseusState = function(state) {
    ...
    state.cards.forEach(function(card, id) {
        if (!cards[id]) {
            console.log('Creating card entity:', id, 'Value:', card.value, 'Color:', card.color, 'FaceUp:', card.faceUp);
            ...
            cardEntity.model.material = card.faceUp ? faceUpMaterial : faceDownMaterial;
        ...
        if (cardEntity.faceUpMaterial && cardEntity.faceDownMaterial) {
            cardEntity.model.material = card.faceUp ? cardEntity.faceUpMaterial : cardEntity.faceDownMaterial;
        }
...
function updatePileLabels() {
    ...
    var cameraMoved = Math.abs(currentCameraPos.x - lastCameraPosition.x) > labelUpdateThreshold ||
                     Math.abs(currentCameraPos.y - lastCameraPosition.y) > labelUpdateThreshold ||
    ...
    lastCameraPosition = { x: currentCameraPos.x, y: currentCameraPos.y, z: currentCameraPos.z };
    ...
    Object.keys(piles).forEach(function(pileId) {
        ...
        if (screenPos.z > 0) { // Only show if in front of camera
            ...
            if (pileEntity.pileType === 'dutch') {
                ...
                else if (pileId.includes('dutch_visible') || pileId.includes('dutch_pile_visible')) labelText = 'DUTCH CARDS';
...
function updateGameUI(state) {
    ...
    // Connection status
    if (!window.room) {
        html += '<p style="color: red;"><strong>Status:</strong> Connecting to server...</p>';
        ...
        html += '<p style="color: yellow;"><strong>Status:</strong> Connected, waiting for game state...</p>';
    } else {
        html += '<p style="color: green;"><strong>Status:</strong> ' + (state.gameStatus || 'waiting') + '</p>';
    }
    ...
    html += 'R - Draw from Wood pile into empty Post slots</p>';
        ...
        html += '<p><strong>Winner:</strong> Player ' + (state.winner || 'unknown') + '</p>';
        ...
        if (state.players && typeof state.players.forEach === 'function') {
            state.players.forEach(function(player, playerId) {
                ...
                html += '<p style="color: ' + scoreColor + ';">' + 
                       (isWinner ? '👑 ' : '') + 'Player ' + playerId.substring(0, 6) + ': ' + 
                       (player.score || 0) + ' points</p>';
            });
        }
